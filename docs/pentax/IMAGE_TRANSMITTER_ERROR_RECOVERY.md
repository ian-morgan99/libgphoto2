...
| Fetch live-view frame (`0x9006`) | A 33 ms periodic callback prevents overlapping requests. Exceptions are swallowed for that frame; polling continues. A non-OK response simply emits no refreshed-frame event. | Bound consecutive failures and elapsed time; surface a diagnostic, stop polling, and restore state after the threshold. Infinite silent polling is unacceptable. |
