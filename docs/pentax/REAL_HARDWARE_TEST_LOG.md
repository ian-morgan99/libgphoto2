...
| K-1 II single-preview attempt | Exact-model research build, dedicated one-frame in-memory harness, 45-second timeout | Vendor enable `0x2001`, flags 3; composite preview returned `GP_ERROR_IO`, no JPEG, no retry; error-path exit ran and USB released | FAIL-CLOSED. Substage and restoration response are not yet separately observable; instrument `0xd009`/`0xd035`/`0x9006`/restore/disable before retry. See H1.12. |
