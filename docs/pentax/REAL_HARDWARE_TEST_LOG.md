...
| Second bounded Near diagnostic | Same one-command/no-retry envelope after compiling raw response messages around `0x900f` and `0x9017` | Returned `GP_ERROR_IO`; neither instrumented setter message appeared; Far was withheld. The artifact was verified to contain the messages | Failure occurred before the instrumented status/focus calls, but the harness did not yet distinguish init from action retrieval. Add host-stage labels before any third attempt; second movement observation pending |
