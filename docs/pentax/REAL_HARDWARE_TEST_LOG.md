...
| Controlled clean power-on lifecycle | Confirm camera absent and all prior sessions terminated; user powered on to normal shooting screen; verify `25fb:0189` and exclusive ownership; make one fresh connection and read shutter descriptor | First `0x9001` enable succeeded with flags 0; `0x1014` read `0xd00f`; current shutter was 1 second with 55 enumerated choices from 1/8000 through 30 seconds; clean exit sent `0x9001` disable and returned success | Confirms the mapping can work from a controlled cold start, but earlier `0x2002` failures make lifecycle repeatability and the differing precondition unresolved |
