...
- **Card writing mode 0x9004 + Green button/interrupt 0x9013: implemented (2026-08-27). Green button HW-verified OK on live K-1 II (2026-09-01, probe `--green` run → raw PTP RC 0x2001); card-write still pending — all four modes return camera-side DeviceBusy (0x2019) while SD2 slot is empty; next test: insert SD2 card and re-run.**
